...
static void my_application_activate(GApplication* application) {
  ...
  if (use_header_bar) {
    ...
    gtk_header_bar_set_title(header_bar, "hackdeepwikireader");
    gtk_header_bar_set_show_close_button(header_bar, TRUE);
    gtk_window_set_titlebar(window, GTK_WIDGET(header_bar));
  } else {
    gtk_window_set_title(window, "hackdeepwikireader");
  }
  ...
  // WebKitGTK is a native GTK widget. Put Flutter in its final GtkOverlay
  // before either GL view is realized; moving an already-realized FlView is
  // what caused the earlier X11 BadAccess/GLX crash.
