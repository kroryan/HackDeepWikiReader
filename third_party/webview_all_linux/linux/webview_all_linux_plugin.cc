...
static void update_flutter_view_input_region(WebviewAllLinuxPlugin* self) {
  // FlView and WebKitGTK are windowless GTK widgets sharing the same X11
  // toplevel. Applying an input shape to either widget therefore shapes the
  // entire application window and makes clicks fall through to the desktop.
  // GtkOverlay already performs the correct child hit testing (WebKit is an
  // overlay child above FlView), so no X11 input-region manipulation is
  // necessary or safe here.
  (void)self;
}
...
static LinuxWebView* create_linux_webview(WebviewAllLinuxPlugin* self) {
  ...
  gtk_overlay_set_overlay_pass_through(overlay, GTK_WIDGET(webview->web_view),
                                       TRUE);
  gtk_widget_set_sensitive(GTK_WIDGET(webview->web_view), FALSE);
  gtk_widget_set_can_focus(GTK_WIDGET(webview->web_view), FALSE);
